...
#############################
packages <- c("tidyverse", "MetBrewer", "arrow", "readstata13", "data.table", "autoCovariateSelection")
...
file_path <- file.path(Denominator, "CPRD Aurum", "Code browsers", "2022_05" , "CPRDAurumProduct.txt")
code_browser <- read.delim(file_path, sep = "\t", header = TRUE, colClasses = "character") %>% dplyr::select(ProdCodeId, dmdid, BNFChapter, Term.from.EMIS, DrugIssues)
...
code_map$bnf_paragraph <- case_when(
  ...
  !is.na(code_map$BNFChapter) & nchar(code_map$BNFChapter) == 7 ~ str_pad(substr(code_map$BNFChapter, 1, 5), 6, side = "left", pad = "0"),
...
code_map$bnf_paragraph <- case_when(
  is.na(code_map$bnf_paragraph) & grepl("paracetamol.*mg.*", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "040701",
  is.na(code_map$bnf_paragraph) & grepl("mirtazapine.*tablets", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "040304",
  is.na(code_map$bnf_paragraph) & grepl("apixaban.*tablets.*", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "020802",
  is.na(code_map$bnf_paragraph) & grepl("aspirin.*mg", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "020900",
  is.na(code_map$bnf_paragraph) & grepl("rosuvastatin.*tablets", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "021200",
  is.na(code_map$bnf_paragraph) & grepl("candesartan.*tablets", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "020505",
  is.na(code_map$bnf_paragraph) & grepl("quinine bisulfate", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "050401",
  is.na(code_map$bnf_paragraph) & grepl("epimax.*cream", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "212200",
  is.na(code_map$bnf_paragraph) & grepl("sitagliptine.*tablets", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "060102",
  is.na(code_map$bnf_paragraph) & grepl("ventolin", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "030101",
  is.na(code_map$bnf_paragraph) & grepl("amlodipin.*tablets.*mg", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "020602",
  is.na(code_map$bnf_paragraph) & grepl("gaviscon", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "010102",
  is.na(code_map$bnf_paragraph) & grepl("erythromycin", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "050105",
  is.na(code_map$bnf_paragraph) & grepl("fluticasone.*nasal spray", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "120201",
  is.na(code_map$bnf_paragraph) & grepl("salbutamol aerosol", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "030101",
  is.na(code_map$bnf_paragraph) & grepl("co-codamol", code_map$Term.from.EMIS, ignore.case = TRUE) ~ "040701",
...
all_codes <- c(ics_single_codes, laba_single_codes, lama_single_codes, laba_lama_codes, ics_laba_codes, triple_therapy_codes)

setwd(Copd_aurum_extract)
# List of drug files to process
drug_files <- list.files(Copd_aurum_extract, pattern = "Drugs_for_hdps_.*\\.parquet", full.names = TRUE)
