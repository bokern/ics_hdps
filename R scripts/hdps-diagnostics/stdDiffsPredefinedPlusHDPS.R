...
# Set paths
data_path <- "outputs/HDPS_100_SMD_covid_hes_present.csv"
...
data <- data %>%
  ...
  #arrange by covariate type and then by unweighted SMD
...
custom_breaks <- c(seq(min(data$adjusted_rank), -1.5, by = 1.5), 0, seq(50, max(data$adjusted_rank), by = 50))
