...
cohort <- "main"
# Load data
data <- read.csv(paste0(HDPS_folder, "/outputs/covariates_HDPS_250_main.csv")) #%>% 

#variable code contains the dimension as the last 2 characters
data$dimension <- substr(data$code, nchar(data$code)-1, nchar(data$code))

 # select(description, dim, tot, percent)
# Order data:
dim <- "x"

#count total number of codes from each dimension
data_dims <- data %>% group_by(dimension) %>% summarise(tot = n())

data <- data %>% arrange(dimension, tot) 
...
# Save
ggsave(paste0(HDPS_folder, "diagnostics/conceptsPlot_", cohort, ".pdf", device = "pdf")
