...
#set up topvars as a list of all values between 100 and 250
topVars <- seq(1, 250, 1)
outcomes <- "covid_hes_present"
cohort_exts <- "_no_triple"
 ...
 cov_weighting <- function(params) {
  ...
  write.csv(
    ...
    file = paste0(HDPS_folder, "/outputs/HDPS_", outcome,"_predefined", cohort_ext, "_trimming_results_check.csv"),
  )
  ...
  write.csv(
    ...
    file = paste0(HDPS_folder, "/outputs/HDPS_", outcome, k, cohort_ext, "_trimming_results_check.csv"),
  )
  ...
  file_path <- file.path(
    HDPS_folder,
    "outputs",
    paste0("HDPS_weight_dist_", k, outcome, cohort_ext, "_trimmed_check.png")
  )
...
# List all trimming_results files
trimming_results <- list.files(
  path = paste0(HDPS_folder, "/outputs"),
  pattern = "trimming_results_check.csv",
  full.names = TRUE
)

# Function to safely read CSV and return NULL if there's an error
...
# Read all CSV files
trimming_results_list <- lapply(trimming_results, safe_read_csv)

# Remove any NULL elements (files that couldn't be read)
trimming_results_list <- Filter(Negate(is.null), trimming_results_list)

# Check if all dataframes have the same column names
col_names <- lapply(trimming_results_list, names)
if (length(unique(col_names)) != 1) {
  cat("Warning: Not all dataframes have the same column names\n")
  for (i in seq_along(col_names)) {
    cat("File", i, "columns:", paste(col_names[[i]], collapse = ", "), "\n")
  }
}

# Attempt to bind rows
tryCatch({
  trimming_results_full <- bind_rows(trimming_results_list)
  cat("Successfully combined all dataframes\n")
  cat("Final dimensions:", nrow(trimming_results_full), "rows,", ncol(trimming_results_full), "columns\n")
  
  # Export to CSV
  write_csv(trimming_results_full, paste0(HDPS_folder, "/outputs/trimming_results_full_check.csv"))
  cat("Successfully wrote combined data to CSV\n")
}, error = function(e) {
  cat("Error in bind_rows or write_csv:\n")
  cat("Error message:", conditionMessage(e), "\n")
  
  # If there's an error, print more detailed information about each dataframe
  cat("\nDetailed information about each dataframe:\n")
  for (i in seq_along(trimming_results_list)) {
    cat("Dataframe", i, ":\n")
    print(names(trimming_results_list[[i]]))
    print(head(trimming_results_list[[i]]))
    cat("\n")
  }
})
